...
#include<cstring>
using namespace std;
int resultz[405],resultx[405],count1;
int xplus(char *s,char *t,int &count)
{
    int add,k;
    int i,j;
    char  sx[405],tx[405],temp[405];
    count=404;
    ...
    if (i<0) {count=404;i=0;}
    for(i=0;i<=404-count-1;i++)
        sx[i]=temp[404-i];
    count1=404;
    ...
    if (j<0){count1=404;j=0;}
    for(j=0;j<=404-count1-1;j++)
        tx[j]=temp[404-j];
    while(i<j)
    {
        if (i>0)
...
void zplus(char *s,char *t,int add,int count)
{
   ...
   if (count1!=404) tlen--;
   if (count!=404)  slen--;
   for(i=slen+count-405,j=tlen+count1-405;i>=0&&j>=0;i--,j--)
   {
       add+=s[i]+t[j]-'0'-'0';
