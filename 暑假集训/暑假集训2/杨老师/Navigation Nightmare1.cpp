/*
算法分析：首先确定要使用的基本算法 是并查集
具体的 就是算出两个点的相对距离 可以对每一个集合 选一个代表点作为原点来确定坐标  若是遇到两个集合有关联 
则按x确定y的  其中坐标一个个点可以从原点开始 传递递推这个很关键   还有就是合并两个代表点 y点坐标的计算 
直接关联的两个点的坐标的距离 即代表两个代表点 除了实际距离以外的距离 相加就可以 解决合并坐标的问题   

后话：这个题 要我自己做 可能不会 即便是做出来 也要耗费大量的时间
*/
#include<iostream>
#include<cstdio>
#include<cmath>
