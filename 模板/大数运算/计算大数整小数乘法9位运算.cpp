...
string multiplication( string &multiplicand,string &multiplicator)
{
   ...
   memset(get_tor,0,sizeof(get_tor));
    count=1;
	while (multiplicand.length()>=9)
	{
		str=multiplicand.substr(multiplicand.length()-9);
		multiplicand.erase(multiplicand.length()-9);
	...
	count=1;
	while (multiplicator.length()>=9)
	{
		str=multiplicator.substr(multiplicator.length()-9);
		multiplicator.erase(multiplicator.length()-9);
